...
public class PlantingServiceImpl implements PlantingService {
	...
	@Override
	public Farmer savePlantingParams(Farmer farmer, String array,
			Integer seedlings) {
		int optimum = 2380;
	...
	public String addTypesOfSeedlingsToJSONArray(Farmer farmer, String array) {
		...
		List<PlantationSeedling> mySeedls = PlantationSeedling.find(
		...
		for (int i = 0; i < mySeedlPercs.length; i++) {
			mySeedlPercs[i] = mySeedls.get(i).percentOfPlantedArea
					* jsonAr.size() / 100;
		}
		int globalCounter = 0;
		for (int i = 0; i < mySeedlPercs.length; i++) {
