...
public class Weather extends Controller {
	...
	private static Double normalize(Double result, Double minValue,
			Double maxValue) {
	...
	/**
	 * mathod that generates random number with biasing if low = 0.0, high = 1.0
	 * and bias = 1.0 the average is 0.5 if low = 0.0, high = 1.0 and bias = 2.0
	 * the average is 0.25 if low = 0.0, high = 1.0 and bias = 0.5 the average
	 * is 0.75
	 * 
	 * @param low
	 * @param high
	...
	public static HashMap<Integer, Double> generateValuesForHashSetBadYear(
			Integer endYear) {
	...
	public static Day generateHumidity(Double humAvg, Day day) {
		if (day.weatherType.id == WEATHER_TYPE_RAINY
				|| day.weatherType.id == WEATHER_TYPE_ICY) {
	...
	public static Day generateHumidityOfLeaf(Double humLeaf, Day day) {
		if (day.weatherType.id == WEATHER_TYPE_RAINY) {
			humLeaf = normalize((humLeaf + random(-200.0, 200.0, 0.75)),
					-200.0, 1640.0) * 1400;
		} else {
			humLeaf = normalize((humLeaf + random(-200.0, 200.0, 1.25)),
					-200.0, 1640.0) * 1400;
		}
	...
	public static Day createWeatherForDay(Integer year, String line,
		...
		if (iceProb == 0.0) { // it is not winter or spring
			...
			if (rainProb < 0.25) {
				if (rainProb > 0.15) {
					// it's cloudy
					WeatherType wType = WeatherType
							.findById(WEATHER_TYPE_CLOUDY);
					day.weatherType = wType;
				} else {
					WeatherType wType = WeatherType
							.findById(WEATHER_TYPE_SUNNY);
					day.weatherType = wType;
				}
			} else {
	...
	public static void generateWeather(String startYear, String endYear) {
		...
		generateValuesForHashSetBadYear(endYearInt);
