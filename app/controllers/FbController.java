...
public class FbController extends Controller {
	public static void login(String signed_request){
		...
		try {
			facebookSR = FacebookSignedRequest.getFacebookSignedRequest(signed_request);
		} catch (Exception e) {
		...
		if(oauthToken == null) {
			...
			JsonElement profilepic=
					WS.url("https://graph.facebook.com/"+user.getId()+"/picture?type=normal&redirect=false").get().getJson();
			
			String picture = profilepic.getAsJsonObject().get("data").getAsJsonObject().get("url").getAsString();
			FarmerService farmerService = new FarmerServiceImpl();
			Farmer farmer = farmerService.buildFbInstance(user.getId(), oauthToken, user.getFirstName(),
						user.getLastName(), user.getEmail(), picture);
